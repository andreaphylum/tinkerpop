...
|enableBulkedResult |Sets whether the result from server should attempt to get bulked. |false
...
Transactions with Java isn't currently supported.
...
4.0, only GraphBinary will be supported in the Gremlin Driver. It is important that the client and server have the same
serializers configured in the same way or else one or the other will experience serialization exceptions and fail to
always communicate. Discrepancy in serializer registration between client and server can happen fairly easily as
different graph systems may automatically include serializers on the server-side, thus leaving the client to be
configured manually. As an example:
