...
|Accept |Serializer MIME types supported for the response. Must be a mimetype (see table below). |No |`application/vnd.gremlin-v4.0+json;types=false`
...
|=========================================================
|Name |Options
|mimetype | application/vnd.gremlin-v4.0+json;types=false - GraphSON without embedded types
|         | application/vnd.gremlin-v4.0+json;types=true - GraphSON with embedded types
|         | application/vnd.graphbinary-v4.0 - GraphBinary with embedded types
...
|         | application/vnd.gremlin-v4.0+json;types=true - GraphSON with embedded types
|         | application/vnd.graphbinary-v4.0 - GraphBinary with embedded types
|         | application/json - some error cases may return JSON back
...
duplicated and should be the same, so graph driver providers should use whichever is easier for them. The trailers, however, will only contain the `Status` and `Exception` without the `Message`.
...
link:https://tinkerpop.apache.org/docs/3.7.3/dev/io/#_responsemessage[GraphSON responses].
...
|language |The script's language.  The default is `gremlin-lang`.
|timeoutMs |The maximum time a query is allowed to execute in milliseconds.  The default value is server dependent.
|materializeProperties |Determines if properties will be included in `Element` results.
|bindings |A `Map` of arbitrary parameters to pass to the server.  The requirements for the contents of this `Map` are dependent on the `op` selected.
|g |The name of the `GraphTraversalSource` to use on the server.  The default is `g`.
|=========================================================
...
HTTP, Gremlin Server expects that the "Content-Type" header will contain the MIME type name of the serializer.
`application/json` should be used for plain JSON requests, such as the one used in the above example.
...
form a single `ResponseMessage`.  A `ResponseMessage` looks like this:
...
|result | The `result` contains a `Map` with one key: `data` which refers to the actual data returned from the server (the type of data is determined by the operation requested). The `data` is always within a `List`.
|=========================================================
...
Note that this is the expected response if the `content-type` is a GraphSON or GraphBinary MIME type. Another issue may
have occurred that prevented the server from returning a serialized response, in which case the serializer will use the
`content-type: application/json`. Be sure to parse the response based on the `content-type` header.

Gremlin Server is capable of streaming results, with additional responses arriving over HTTP in chunks until the
iteration of the result on the server is complete.  The following table details the HTTP status codes that Gremlin
Server will send:
...
|505 |HTTP VERSION NOT SUPPORTED |A server indicating that an unsupported version of HTTP is being used. Only HTTP/1.1 is supported.
|=========================================================
...
should have access to a mechanism that allows them to update the HTTP headers and HTTP body of the HTTP request before
the request is sent to the server.  This allows for use cases where a graph system provider's server implementation has
additional capabilities that aren't included in the base Gremlin Server.  Although every graph system provider is
expected to support the protocol defined by the TinkerPop HTTP API, this doesn't preclude them from including
additional functionality.  The graph drivers should, therefore, be flexible enough to allow the use of those additional
capabilities, as long as the response follows the proper serialized `ResponseMessage` format.  There are no specific
implementation requirements for request interceptors.  The graph driver provider just needs to provide some means of
...
to round trip every type. The GraphBinary test files can be found
